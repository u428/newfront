...
import {
    ...
    STUDENT_PAYMENT,
    GET_HISTORY_STUDENT,
    VIEW_STUDENTS_GROUP, DELETE_STUDENT_FROM_GROUP
} from "../actions";
import {
    fetchGetStudentList,
    fetchGetNewStudentList,
    fetchGetSingleStudent,
    fetchDeleteStudent,
    fetchPostStudent,
    fetchPutStudent,
    fetchPostStudentLogin,
    fetchPostStudentGroup,
    fetchGetSingleStudentGroups,
    fetchGetStudentGroupList,
    fetchPaymentStudent,
    fetchHistoryStudent,
    fetchViewStudentGroupList, fetchDeleteStudentFromGroup
} from "../services/api";
import {notificationMessage} from "../services/notificationMessage";
import {
    getNewStudentsSuccess,
    getNewStudentsError,
    postStudentGroupError,
    postStudentGroupSuccess,
    postStudentLoginError,
    postStudentLoginSuccess,
    getStudentsError,
    getStudentsSuccess,
    getSingleStudentSuccess,
    getSingleStudentError,
    deleteNewStudentSuccess,
    deleteNewStudentError,
    postNewStudentSuccess,
    postNewStudentError,
    putNewStudentSuccess,
    putNewStudentError,
    getSingleGroupStudentSuccess,
    getSingleGroupStudentError,
    getStudentsGroupSuccess,
    getStudentsGroupError,
    studentPaymentSuccess,
    studentPaymentError,
    getHistoryStudentSuccess,
    getHistoryStudentError,
    viewStudentsGroupSuccess, viewStudentsGroupError, deleteStudentFromGroupSuccess, deleteStudentFromGroupError
} from "./actions";
...
function* watchViewStudentsGroup() {
    yield takeEvery(VIEW_STUDENTS_GROUP, workViewStudentsGroup);
}
...
function* workViewStudentsGroup({payload}) {
    ...
    if (response) {
        ...
        history.push("/admin/view/group")
    } else {
        yield put(viewStudentsGroupError(error.response.data));
        notificationMessage("error", error.response.data);
    }
}
...
function* watchDeleteStudentFromGroup() {
    yield takeEvery(DELETE_STUDENT_FROM_GROUP, workDeleteStudentFromGroup);
}

function* workDeleteStudentFromGroup({payload}) {
    const {response, error} = yield call(fetchDeleteStudentFromGroup, payload.studentId, payload.groupId);
    if (response) {
        yield put(deleteStudentFromGroupSuccess(response.data));
    } else {
        yield put(deleteStudentFromGroupError(error.response.data));
        notificationMessage("error", "SORRY: there is some ERRORS");
    }
}
...
export default function* studentSaga() {
    yield all([
        ...
        fork(watchDeleteStudentFromGroup),
        fork(watchViewStudentsGroup)
    ]);
}
